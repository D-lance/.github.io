...
def new_file(test_dir):
    ...
    # 最后对lists元素，按文件修改时间大小从小到大排序。windows系统目录分隔符为'\\'，Mac为'/'
...
if __name__=='__main__':
    ...
    try:
        ...
        print(e)
    filename = report_dir + '/' + 'android_report_' + now + '.html'
    print(u'测试报告名称：' + filename)
    # 需屏蔽fp中的中文文字说明。
    fp = open(filename, 'wb')
    runner = HTMLTestRunner(stream=fp, title=u'Android_TestReport', description=u'Case Results：', verbosity=2, retry=0, save_last_try=False)
    runner.run(discover)
    print(discover)
    # 注意：调用函数一定要加括号
    fp.close()
